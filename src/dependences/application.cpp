...
static T_DjiHalI2cHandler s_i2cHandler = {0};
...
void Application::DjiUser_SetupEnvironment()
{
    ...
    s_i2cHandler.I2cInit = HalI2c_Init;
    s_i2cHandler.I2cDeInit = HalI2c_DeInit;
    s_i2cHandler.I2cWriteData = HalI2c_WriteData;
    s_i2cHandler.I2cReadData = HalI2c_ReadData;
...
void Application::DjiUser_ApplicationStart()
{
    ...
    returnCode = DjiPlatform_RegHalI2cHandler(&s_i2cHandler);
    if (returnCode != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
    ...
    /**
     * @date 2025/11/05
     * 在测试云台角度的数据订阅时报错，说这里要设置一个custom serial num、
     * 于是我将原本的程序注释，写一段新的
     */
...
T_DjiReturnCode Application::DjiUser_FillInUserInfo(T_DjiUserInfo *userInfo)
{
    ...
    if (!strcmp(USER_APP_NAME, "your_app_name") ||
        ...
        USER_LOG_ERROR(
            "Please fill in correct user information in "
    ...
    printf("========== DJI APP INFO ==========\n");
    printf("USER_APP_NAME: %s\n", USER_APP_NAME);
    printf("USER_APP_ID: %s\n", USER_APP_ID);
    printf("USER_DEVELOPER_ACCOUNT: %s\n", USER_DEVELOPER_ACCOUNT);
    printf("USER_BAUD_RATE: %s\n", USER_BAUD_RATE);
    printf("USER_PSDK_ALIAS: %s\n", USER_PSDK_ALIAS);
    printf("USER_PSDK_SERIAL_NUMBER: %s\n", USER_PSDK_SERIAL_NUMBER);
    printf("==================================\n");
    fflush(stdout);
